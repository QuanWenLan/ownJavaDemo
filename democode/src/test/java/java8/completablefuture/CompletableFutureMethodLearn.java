 ...
 **/
public class CompletableFutureMethodLearn {
    ...
    // 异步请求可以写在方法表达式里面， 多个异步任务，并行执行，等待所有任务完成
    @Test
    public List<String> findPrices1(String product) {
        ...
        // 耗时 [BestPrice price is 123.26, LetsSaveBig price is 169.47, MyFavoriteShop price
    ...
    public List<String> findPrices3(String product) {
        ...
        List<String> prices = priceFutures.stream()
                .map(CompletableFuture::join) // 等待执行完成
                .collect(Collectors.toList());
        // join不会抛出任何检测到的异常
        return prices;
        // 耗时 [BestPrice price is 123.26, LetsSaveBig price is 169.47, MyFavoriteShop price

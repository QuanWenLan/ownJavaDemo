 ...
 * @Date 2020/4/24 9:33
 */
public class BsDossierRequester extends DataRequester {
    ...
    public BsDossierRequester(String name, RequestContext context) {
        super("bs-dossier-1", "1000000", context);
        this.name = name;
    }

    @Override
    public void run() {
        System.out.println("当前线程任务执行：" + Thread.currentThread().getName() + " " + BsDossierRequester.class.getSimpleName());
        doRequest();
    }

    private void doRequest() {
        for (int i = 0; i < 4; i++) {
            DossierData dossierData = new DossierData(i + "", "业务数据", Arrays.asList(i + 1, i + 2, i + 3), true, "");
            resultQueue.add(dossierData);
        }
        System.out.println(BsDossierRequester.class.getSimpleName() + " 数据请求结束。。。。。");
        System.out.println(BsDossierRequester.class.getSimpleName() + "数据收集开始");
        try {
        ...
        System.out.println(BsDossierRequester.class.getSimpleName() + "数据收集结束");
    }
